...
<script>
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4) 
				{
					...
					for(i=0 ; i<jsonObj.length ; i++)
					{
						...
						document.city.state.options.add(createOption);
						
					}
				}
			}
			
			xmlhttp.open("get", "${pageContext.request.contextPath}/cityController?flag=loadstate&countryId="+cid.value, true);
			xmlhttp.send();
		...
		function removeAllState()
		{
			...
			var removeState=document.city.state.options.length;
			for(i=removeState ; i>0 ; i-- )
			{
				document.city.state.remove(i);
			}
		}
...
<%@include file="header.jsp" %>
			...
			<hr>					<div class="panel-body">
										<form class="form-horizontal" name="city" role="form" method="post" action="<%=request.getContextPath()%>/cityController">
										<div class="form-group">
											<label class="col-sm-2 control-label">Country Name</label>
											<div class="col-sm-10">
												<select class="form-control" name="country" id="country_id" onchange="load()">
											...
											<div class="col-md-10">
												<input type="text" class="form-control" requuired="required" placeholder="Enter City Name" name="city" required="">
											</div>
										</div>

										<div class="form-group"> 
											<label class="col-md-2 control-label">City Description</label>
											<div class="col-md-10">
												<textarea placeholder="City Description" class="form-control" rows="5" name="city1" required=""></textarea>
											</div>
										</div>
										<div class="form-group"> 
											<div class="col-md-10">
											<input type="hidden" class="form-control"
													 name="flag" value="add_city">
												</div> 
											</div>
